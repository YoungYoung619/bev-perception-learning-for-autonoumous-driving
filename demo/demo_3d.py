...
def parse_args():
    ...
    parser.add_argument('--version', type=str, help='dataset name',
                        default='mini'
                        )
    parser.add_argument('--dataset_root', type=str, help='dataset root path',
                        default='/Volumes/KINGSTON0/nuscenes',
                        )
...
def main():
    ...
    while cur_idx < len(dataset):
        ...
        if has_box3d(outs):
            ...
            # save_root = '/Volumes/KINGSTON0/visualization/bev_3d'
    ...
    # person_conf = 0.29
    # face_key = 'face'
    # dynamic_obj_key = 'moving_obstacle'
    # batch_idx = 0
    # logger.log("Using key . and , to traverse images, and q for exit...")
    # while cur_idx < len(files):
    #     image_name = files[cur_idx]
    #     meta, res = predictor.inference(image_name)
    #
    #     raw_face_bboxes = predictor.get_target_objs(face_key, [0], 1.0, 0.0)
    #     if len(res[dynamic_obj_key][batch_idx]) == 2:  # person vehicle
    #         person_bboxes = predictor.get_target_objs(dynamic_obj_key, [0], 0.5, person_conf)
    #         veh_bboxes = predictor.get_target_objs(dynamic_obj_key, [1], 0.5, 0.4)
    #     elif len(res[dynamic_obj_key][batch_idx]) == 4:  # person rider veh_big veh_small
    #         person_bboxes = predictor.get_target_objs(dynamic_obj_key, [0, 1], 0.5, person_conf)
    #         veh_bboxes = predictor.get_target_objs(dynamic_obj_key, [2, 3], 0.5, 0.4)
    #     else:
    #         raise NotImplementedError
    #
    #     # face pipeline
    #     two_stage_bboxes = face_two_stage_predictor.inference(meta['raw_img'][batch_idx], person_bboxes,
    #                                                           face_two_stage_val.expand_ratios)
    #     all_face_bboxes = raw_face_bboxes + two_stage_bboxes
    #     all_face_bboxes = [box for box in all_face_bboxes if
    #                        box[-1] > eval_config[args.standard_type]['one_stage_conf_thresh']]
    #     keep = nms(np.array(all_face_bboxes).astype(np.float32), 0.01)
    #     all_face_bboxes = np.array(all_face_bboxes).astype(np.float32)[keep].tolist()
    #     if args.with_desensitive_cls_model:
    #         all_face_bboxes = desensitive_cls_predictor.inference(meta['raw_img'][batch_idx], all_face_bboxes, 0.)
    #     predictor.visualize_face(all_face_bboxes, meta, eval_config[args.standard_type]['face_cls_conf'])
    #
    #     # licence pipeline
    #     two_stage_bboxes = licence_two_stage_predictor.inference(meta['raw_img'][batch_idx], veh_bboxes,
    #                                                              expand_ratios=0.)
    #     predictor.visualize_licence(two_stage_bboxes, meta, 0.)
    #
    #     if 'moving_obstacle' in res and args.vis_moving_obstacle:
    #         predictor.visualize_moving_obstacle(res['moving_obstacle'][0], meta,
    #                                             ['ped', 'car'], [person_conf, 0.4])
    #
    #     if args.save_result:
    ...
    #         cv2.imwrite(save_file_name, meta['raw_img'][0])
    #         logger.log("saving image to: %s" % save_file_name)
    #         cur_idx += 1
    #         continue
    #
    #     cv2.imshow("test", meta['raw_img'][batch_idx])
    #     key = cv2.waitKey(0)
    #     if key == ord('.'):
    #         cur_idx += 1
    #     elif key == ord(','):
