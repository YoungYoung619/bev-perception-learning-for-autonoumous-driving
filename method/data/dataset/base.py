...
def load_bin_file(self, index):
    mask = None
    img_path = os.path.join(self.img_path, self.data_info[index]['file_name'])
...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def __init__(
            ...
            pipeline: Dict = None,
            ann_path: str = None,
            source_ann_path: str = None,
            target_ann_path: str = None,
        ...
        self.pipeline = self.build_pipeline(kwargs['name'], input_size, pipeline, keep_ratio)
        ...
        self.mode = mode
        self.need_cache_data = cache_data
        self.with_depth = with_depth
        self.logger = logger
        self.rank = self.logger.rank
        self.source_ann_path = source_ann_path
        self.target_ann_path = target_ann_path
    ...
    def build_pipeline(self, dataset, input_size, pipeline_cfg, keep_ratio):
        return Pipeline(dataset, input_size, pipeline_cfg, keep_ratio)

    def __len__(self):
        return len(self.data_info) if self.data_info else len(self.idx_2_sample_tokens)
    ...
    @abstractmethod
    def get_data_info(self, ann_path, **kwargs):
        pass
    ...
    def get_cross_domain_flags(self):
        pass
