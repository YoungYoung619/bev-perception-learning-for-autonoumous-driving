...
from method.model.backbone.mobile_former.dna_block import (
    DnaBlock,
    DnaBlock3,
    _make_divisible,
    MergeClassifier,
...
__all__ = ['build_mobile_former']

model_urls = {
    "mobile_former_508m": 'https://mirrors.tencent.com/repository/generic/model_packages/mobile-former/mobile-former-508m.pth.tar',
    "mobile_former_294m": 'https://mirrors.tencent.com/repository/generic/model_packages/mobile-former/mobile-former-294m.pth.tar',
    "mobile_former_214m": 'https://mirrors.tencent.com/repository/generic/model_packages/mobile-former/mobile-former-214m.pth.tar',
    "mobile_former_151m": 'https://mirrors.tencent.com/repository/generic/model_packages/mobile-former/mobile-former-151m.pth.tar',
    "mobile_former_96m": 'https://mirrors.tencent.com/repository/generic/model_packages/mobile-former/mobile-former-96m.pth.tar',
    "mobile_former_52m": 'https://mirrors.tencent.com/repository/generic/model_packages/mobile-former/mobile-former-52m.pth.tar',
    "mobile_former_26m": 'https://mirrors.tencent.com/repository/generic/model_packages/mobile-former/mobile-former-26m.pth.tar',
}
    ...
    def _initialize_weights(self, pretrain=False):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
        if pretrain:
            url = model_urls["mobile_former_{}".format(self.model_size)]
            if url is not None:
...
def _create_mobile_former(model_size, out_stages, **kwargs):
    model = MobileFormer(model_size, out_stages, **kwargs)
    return model
...
def mobile_former_508m(out_stages, pretrained=False, **kwargs):
    # stem = 24
    dna_blocks = [
        # b, e1,  c, n, s, e2
    ...
    model = _create_mobile_former('508m', out_stages, pretrain=pretrained, **model_kwargs)
    return model


def mobile_former_294m(out_stages, pretrained=False, **kwargs):
    # stem = 16
    dna_blocks = [
        # b, e1,  c, n, s, e2
    ...
    model = _create_mobile_former('294m', out_stages, pretrain=pretrained, **model_kwargs)
    return model


def mobile_former_214m(out_stages, pretrained=False, **kwargs):
    # stem = 12
    dna_blocks = [
        # b, e1,  c, n, s, e2
    ...
    model = _create_mobile_former('214m', out_stages, pretrain=pretrained, **model_kwargs)
    return model


def mobile_former_151m(out_stages, pretrained=False, **kwargs):
    # stem = 12
    dna_blocks = [
        # b, e1,  c, n, s, e2
        ['DnaBlock3', 2, 12, 1, 1, 0],  # 1 112x112 (1)
    ...
    model = _create_mobile_former('151m', out_stages, pretrain=pretrained, **model_kwargs)
    return model


def mobile_former_96m(out_stages, pretrained=False, **kwargs):
    # stem = 12
    dna_blocks = [
        # b, e1,  c, n, s, e2
        ['DnaBlock3', 2, 12, 1, 1, 0],  # 1 112x112 (1)
    ...
    model = _create_mobile_former('96m', out_stages, pretrain=pretrained, **model_kwargs)
    return model


def mobile_former_52m(out_stages, pretrained=False, **kwargs):
    # stem = 8
    dna_blocks = [
        # b, e1,  c, n, s, e2
    ...
    model = _create_mobile_former('52m', out_stages, pretrain=pretrained, **model_kwargs)
    return model


def mobile_former_26m(out_stages, pretrained=False, **kwargs):
    # stem = 8
    dna_blocks = [
        # b, e1,  c, n, s, e2
        ['DnaBlock3', 3, 12, 1, 2, 0],  # 1 56x56 (2)
    ...
    model = _create_mobile_former('26m', out_stages, pretrain=pretrained, **model_kwargs)
    return model

def build_mobile_former(model_size, out_stages=[], pretrain=False, **kwargs):
    assert model_size in ['508m', '294m', '214m', '151m', '96m', '52m', '26m']

    model_func = 'mobile_former_%s' % model_size
    model = eval(model_func)(out_stages, pretrain, **kwargs)
    return model


if __name__ == '__main__':
    from method.util.flops_counter import get_model_complexity_info

    # model = mobile_former_151m()
    # model = mobile_former_96m()
    # model = mobile_former_52m()
    model = build_mobile_former('52m', out_stages=[3, 6, 8], pretrained=True)
    input_shape = (3, 256, 448)
    print(get_model_complexity_info(model, input_shape,
                                    print_per_layer_stat=False,
                                    as_strings=False))

    #
    # resume_file = '/Users/lvanyang/Downloads/mobile-former-26m.pth.tar'
    # resume_dicts = torch.load(resume_file, map_location=lambda storage, loc: storage)
    # model.load_state_dict(state_dict, strict=True)
    #
    #
    # aa = model_zoo.load_url('https://mirrors.tencent.com/repository/generic/model_packages/mobile-former/mobile-former-26m.pth.tar',
