...
class ATSSAssigner(BaseAssigner):
    ...
    def __init__(self, topk, xbound, ybound):
        self.topk = topk
        self.xbound = xbound
        self.ybound = ybound
    ...
    def assign(
            self, bboxes, num_level_bboxes, gt_bboxes_3d, gt_bboxes_ignore=None, gt_labels=None
    ):
        """Assign gt to bboxes.
        ...
        # 首先将尺度转换成鸟瞰图大小200x200，然后方向变成鸟瞰图坐标系方向(lss中，鸟瞰图高度增加方向为x正向)
        # 注：如果后续有别的方法鸟瞰图的坐标方向和lss中不一致，这里就需要调整
        # convert gt_bboxes to x1 y1 x2 y2 format, we use x y w l to represent the gt plate box without rotation and height dimensions
        x, y, w, l = gt_bboxes_3d[:, 0], gt_bboxes_3d[:, 1], gt_bboxes_3d[:, 3], gt_bboxes_3d[:, 4]
        x1 = ((x - self.xbound[0]) - l / 2.) / self.xbound[2]
        y1 = ((y - self.ybound[0]) - w / 2.) / self.ybound[2]
        x2 = ((x - self.xbound[0]) + l / 2.) / self.xbound[2]
        y2 = ((y - self.ybound[0]) + w / 2.) / self.ybound[2]
        gt_bboxes = torch.stack((y1, x1, y2, x2), dim=-1)
