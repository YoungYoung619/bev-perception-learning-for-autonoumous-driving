...
from method.util import bbox2distance, distance2bbox, multi_apply, overlay_bbox_cv
...
class NanoDetPlusHead(nn.Module):
    ...
    def loss(self, preds, gt_meta, valid_idx, key_name_pair=('gt_bboxes', 'gt_labels'), aux_preds=None):
        """Compute losses.
        Args:
        ...
        aux_preds = aux_preds[valid_idx] if aux_preds is not None else aux_preds
        # get target objects
        gt_bboxes, gt_labels = self.get_target_objs(gt_meta, key_name_pair)
    ...
    def post_process(self, preds, meta, valid_idx=None, warp=True):
        ...
        if valid_idx is None:
            ...
            warp_matrixes = [mat for i, mat in enumerate(meta["warp_matrix"]) if i in valid_idx]
            img_heights = meta["img_info"]["height"][valid_idx].cpu().numpy()
            img_widths = meta["img_info"]["width"][valid_idx].cpu().numpy()
        # if "id" not in meta["img_info"]:
        #     bs = preds.shape[0]
        #     meta["img_info"]["id"] = np.arange(0, bs)
        # img_ids = (
    ...
    def get_target_objs(self, gt_meta, key_name_pair=('gt_bboxes', 'gt_labels')):
        """ get target objs for training
        """
        gt_bboxes_all = gt_meta[key_name_pair[0]]
        gt_labels_all = gt_meta[key_name_pair[1]]
        gt_bboxes = []
        gt_labels = []
        for gt_bboxes_batch, gt_labels_batch in zip(gt_bboxes_all, gt_labels_all):
            gt_bboxes_ = [gt_bbox for gt_label, gt_bbox in zip(gt_labels_batch, gt_bboxes_batch) if
                          gt_label in self.det_target]
            gt_labels_ = [self.map_label[self.det_target.index(gt_label)] for gt_label in gt_labels_batch \
                          if gt_label in self.det_target]
            if len(gt_bboxes_) > 0:
                gt_bboxes.append(np.array(gt_bboxes_))
                gt_labels.append(np.array(gt_labels_))
            else:
                # there is no any targets in this img
