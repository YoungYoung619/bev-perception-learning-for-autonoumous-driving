...
class TrainingTask(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # print('train: ', ' rank-', self.local_rank, ' b_id-', batch_idx)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        """
        head_names = model.get_head_names()
        head_types = model.get_head_types()

        dets = {}
        for head_name, head_type in zip(head_names, head_types):
            gt_generator = eval('model.%s.generate_gt' % (head_name))
            gts = gt_generator(batch, batch['imgs'].device)

            post_processor = eval('model.%s.post_process' % (head_name))
            outs = post_processor(preds[head_name[:-5]], batch, True)

            sample_tokens = batch['ann_info']['sample_token']

            if head_type in ['BEVSegmHead']:
                """语义分割头"""
                for gt, out, sample_token in zip(gts, outs, sample_tokens):
                    if isinstance(sample_token, list):
                        sample_token = sample_token[0]
                    gt = gt.cpu().numpy()
                    if sample_token not in dets:
                        dets[sample_token] = {
                            'pred': {},
                            'gt': {},
                        }
                    dets[sample_token]['pred'][head_name[:-5]] = out
                    dets[sample_token]['gt'][head_name[:-5]] = gt
            elif head_type in ['BevCos3DHead', 'BEVCenter3DHead']:
                """3D目标检测头"""
                for batch_idx, boxes_pred in outs.items():
                    sample_token = sample_tokens[batch_idx]
                    if isinstance(sample_token, list):
                        sample_token = sample_token[0]
                    ...
                    if head_name[:-5] not in dets[sample_token]['pred']:
                        dets[sample_token]['pred'][head_name[:-5]] = []
                    elif head_name[:-5] in dets[sample_token]['pred'] and dets[sample_token]['pred'][head_name[:-5]]:
                        # 分布式下, 不知道会不会有重复的数据喂进来, 如果有, 跳过此次重复数据, 应该不会触发
                        import warnings
                        warnings.warn('Validation step recieved repeat data! This is not a bug, feel free to ignored it.')
                        continue

                    for box_pred in boxes_pred:
                        dets[sample_token]['pred'][head_name[:-5]].append(box_pred)
                        dets[sample_token]['gt'][head_name[:-5]] = -1
            else:
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        """
        # a = self.outs_nuscenes_format
        # import json
        # with open('/Users/lvanyang/ADAS/ADMultiTaskPerception/results.json', 'w') as f:
        #     json.dump(a, f)
