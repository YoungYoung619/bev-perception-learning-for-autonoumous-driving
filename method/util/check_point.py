...
def load_model_weight(model, checkpoint, logger, name=None):
    ...
    if name is None:
        ...
        for key, val in state_dict.items():
            prefix = key.split(".")[0]
            if prefix not in resume_state_dicts:
                resume_state_dicts[prefix] = OrderedDict()
            resume_state_dicts[prefix][key[len(prefix)+1:]] = val
        if name not in resume_state_dicts:
            name = 'model'  # default choice
        state_dict = resume_state_dicts[name]
