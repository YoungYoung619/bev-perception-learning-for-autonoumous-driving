...
https://github.com/jw9730/ori-giou
"""
import torch
...
def nms(dets, thresh=0.5):
    device = dets.device
    dets = dets.to('cpu')   # convert to cpu is faster
    if len(dets) == 0:
        return []
    boxes, scores = dets[:, :6], dets[:, 6]
    boxes = box_center_to_corners(boxes)
    order = adaptive_mps_argsort(scores).flip(dims=[0])
    a = scores[order]
    # area = box_area(boxes)
    keep = []
    while order.size()[0] > 0:
        ...
        if order.size()[0] == 1:
            break
        over, _ = box_iou(boxes[i][None, :], boxes[order[1:]])
        index = torch.where(over[0, :] <= thresh)[0]
        order = order[index + 1]  # 不包括第0个
    # keep = torch.concatenate(keep, dim=0)
